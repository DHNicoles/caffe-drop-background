	 ...
	 // }  else 
	  {
		  ...
		  for (int item_id = 0; item_id < batch_size; ++item_id) {
			  ...
			  //LOG(INFO)<<" item_id："<<item_id<<" anno_vec' size: "<<anno_vec.size();
			  ...
			  if(anno_vec.size()==0)
			  {
				  //LOG(INFO)<<"  anno_vec' size: "<<anno_vec.size()<<" if current image has none label bbox: ";
				  //const TransformationParameter&  transform_param = this->layer_param_.transform_param();
				  top_label[idx++] = item_id;
				  top_label[idx++] = 0;
				  top_label[idx++] = 0;
				  top_label[idx++] = 0.0; // xmin
				  top_label[idx++] = 0.0; // ymin
				  top_label[idx++] = 1.0; //transform_param.resize_param().height();
				  top_label[idx++] = 1.0; //transform_param.resize_param().width();
				  top_label[idx++] = 0;
				  //LOG(INFO)<<"  item_id: "<<item_id<<" [height, width]: "<< top_label[5] <<" ,"<< top_label[6];
			  }
		  }
	  }
	} else {
