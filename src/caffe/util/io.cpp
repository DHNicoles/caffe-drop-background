      ...
      BOOST_FOREACH(ptree::value_type &v2, object.get_child("")) {
        ...
        if (v2.first == "name") {
          ...
          for (int g = 0; g < anno_datum->annotation_group_size(); ++g) {
			...
			LOG(INFO)<<"\tlabel: " <<label <<" annotation_group_size: " <<anno_datum->annotation_group_size()<<" annotation_size() "<<anno_group->annotation_size();
            if (label == anno_group->group_label()) {
              if (anno_group->annotation_size() == 0) {
                instance_id = 0;
				LOG(INFO)<< "instance_id: 0";
              } else {
                instance_id = anno_group->annotation(
                    anno_group->annotation_size() - 1).instance_id() + 1;
					//LOG(INFO)<< "\tannotation_size: "<<anno_group->annotation_size() ;
              }
