  ...
  for (int line_id = 0; line_id < lines.size(); ++line_id) {
    ...
    if (anno_type == "classification") {
	  ...
	  for (int g = 0; g < anno_datum.annotation_group_size(); ++g) {
		  ...
		  for(int b=0; b< anno_datum.annotation_group(g).annotation_size(); ++b){
			  const Annotation& anno = anno_datum.annotation_group(g).annotation(b);
			  const NormalizedBBox& bbox = anno.bbox();
			//if( b==1)
			{
				//LOG(INFO)<<" group_label: "<<anno_datum.annotation_group(g).group_label()<<" instance_id: "<<anno.instance_id()
					//<<" xmin: "<<bbox.xmin()<<" ymin: "<<bbox.ymin()<<" xmax: "<<bbox.xmax()<<" ymax: "<<bbox.ymax()<<" diff: "<<bbox.difficult();
			}
		  }
		  //LOG(INFO)<<"group: "<<g << ", boxes num: "<< anno_datum.annotation_group(g).annotation_size();
	  }
	  //LOG(INFO)<<"anno_datum.annotation_group_size: "<<anno_datum.annotation_group_size()<<", size_bbox: "<<num_bboxes;
	}
    if (status == false) {
